...
char MyString::charAt(int index) const {
    ...
    return data [index];// Throw std::out_of_range if index is invalid (negative or >= length)
}
...
MyString MyString::substring(int start, int len) const {
    ...
    return MyString(data.substr(start,len));//TODO: Return a substring starting at 'start' with length 'len'
...
MyString MyString::toUpperCase() const {
    string result = data;// TODO: Return a NEW MyString with all characters converted to upper case
    transform(result.begin(), result.end(), result.begin(),// Hint: Use std::transform with ::toupper
    [](unsigned char c) {return toupper(c);});
    return MyString(result);// Do NOT modify the original object
}
...
MyString MyString::toLowerCase() const {
    ...
    transform(result.begin(), result.end(), result.begin(),// Hint: Use std::transform with ::toupper
    [](unsigned char c) {return toupper(c);});
    return MyString(result);// TODO: Return a NEW MyString with all characters converted to lower case
